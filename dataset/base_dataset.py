# TODO IMPORTANT: find out where to convert video to images or add prepare dataset
import os
import pdb
...
class QueryVideoDataset(Dataset):
    def __init__(self,
                 ...
                 query_params,
                 clip_params,
                 data_dir='./DLCV_vq2d_data/',      # crop the query from image first
                 clip_dir='./DLCV_vq2d_data/clips',
                 meta_dir='./DLCV_vq2d_data/',      # folder that contains annotation json file
                 split='train',
                 clip_reader='decord_balance',      # choice: [decord_balance]
                 eval_vis_freq=50,
                 ):
        ...
        self.video_dir = '/vision/vision_data/Ego4D/v1/full_scale'  # useless

        self.split = split 

        self.clip_reader = video_reader_dict[clip_reader]   # TODO: understand 'decord_random' in "egotracks_dataset.py", and if needed, add it on
        self._load_metadata()
        if self.split != 'train':
            self.annotations = self.annotations[::eval_vis_freq]    # visualization?


    def _load_metadata(self):
        annot_dir_path = "./DLCV_vq2d_data/annot/"
        anno_processed_path = os.path.join(annot_dir_path, '{}_anno_new.json'.format(self.split))
        if os.path.isfile(anno_processed_path):
            ...
            # TODO: modify this to our format (complete)
            self.annotations, n_samples, n_samples_valid = [], 0, 0
            for cuid, clip_data in anno_json.items():
                for aid, clip_anno in enumerate(clip_data['annotations']):  # list
                    for qset_id, qset in clip_anno['query_sets'].items():
                        if not qset['is_valid']:
                            continue
                        response_track_frame_ids = []   # frame ids
                        ...
                        query_path = self._get_query_path(curr_anno, self.split)
                        # if cuid == '859ed253-d752-4f1b-adc3-c76599117d6e':
                        #     print(query_path)
                        if os.path.isfile(query_path):  # TODO: where to make the images
                            self.annotations.append(curr_anno)
                            n_samples_valid += 1
                        elif self.split == 'train' and cuid == '0a8f3f78-4a0b-402e-b5ca-ce61a13adc92':
    ...
    def _get_video_path(self, sample):  # wont use it!!
    ...
    def _get_query_path(self, sample, split=""):
        clip_name = sample['clip_uid']
        image_name = int(sample["visual_crop"]["frame_number"])# "{}/frame_{:07d}.jpg"
        # TODO: change path
        image_path = os.path.join(self.data_dir, 'images',split, "{}/frame_{:07d}.png".format(clip_name, image_name + 1))
        return image_path
    ...
    def _get_video_lens(self):
        ...
        for idx, cur_anno in enumerate(self.annotations):
            video_path = self._get_clip_path(cur_anno)
            video_len_list[video_path] = get_video_len(video_path)  # how many frame in clips
    ...
    def _get_clip_bbox(self, sample, clip_idxs):
        ...
        clip_bbox_all = {}
        for it in response_track:   # get all clip box from all response track
            clip_bbox_all[int(it['frame_number'])] = [it['y'], it['x'], it['y'] + it['height'], it['x'] + it['width']] # in torch
            origin_hw = [int(it['original_height']), int(it['original_width'])] # only assign once = =
        for id in clip_idxs:    # the frame to train. Here check which frame has gt.
            if int(id) in clip_bbox_all.keys():
                ...
                clip_bbox.append(torch.tensor([0.0, 0.0, 0.00001, 0.00001])) # make AP calculated??
    ...
    def _get_query(self, sample, query_path):
        query = Image.open(query_path)  # the query image is not cropped yet
        ...
        # pad query
        # default query_padding is False
        if self.query_params['query_padding']:      # pad to square -> less deformation after resize
            ...
            # resize query
            query_size = self.query_params['query_size']    # resize to 448
    ...
    def _get_query_frame(self, sample, query_path):     # return query frame (not cropped) and bbox
    ...
    def __getitem__(self, idx):
        ...
        query_path = self._get_query_path(sample, self.split)
...
def sample_frames_balance(num_frames, query_frame, frame_interval, sample, sampling='rand'):
    ...
    anno_len = anno_valid_idx_range[1] - anno_valid_idx_range[0] + 1    # len with annotations
    ...
    if anno_len <= required_len:
        ...
        if anno_valid_idx_range[1] < required_len:      # cannot access more previous frame than valid set
        ...
        frame_idxs_pos = [anno_valid_idx_range[0] + start + it for it in range(num_frames)] # start from annotation clip, end to required_len
...
def read_frames_decord_balance(video_path, num_frames, frame_interval, sample, sampling='rand'):
    ...
    before_query = torch.tensor(frame_idxs) < query_frame   # check frames are before query frame
