...
def custom_convert_annotations_to_clipwise_list(annotations):
    clipwise_annotations_list = defaultdict(lambda: [])
    for cuid, c in annotations.items():
        # cuid: string of clip id
        for idx, a in enumerate(c["annotations"]):
            ...
            for qid, q in a["query_sets"].items():
                ...
                if "response_track" in q:
                    # this is for vq_test_unannotated.json
                    curr_q["response_track"] = q["response_track"]
