...
class Task:
    def __init__(self, config, annots, cuid=""):
        ...
        ### Ray: I didnot use clip_uid in annot, so I wont ensure this
    ...
    # for debug
    def get_query(self, config, device):
        clip_uid = self.clip_uid

        if clip_uid is None or clip_uid == "":
            return
        ...
        for annot in self.annots:
            ...
            annot_key = f"{clip_uid}_{query_set}"
            query_frame = annot["query_frame"]
            visual_crop = annot["visual_crop"]
            save_path = os.path.join(self.config.inference_cache_path, f'{annot_key}.jpg')
            # if os.path.isfile(save_path):
            #     print("the same path")
            #     continue

            image = get_query(config, clip_path, query_frame, visual_crop, device)
    ...
    def run(self, model, config, device):
        ...
        for annot in self.annots:
            query_set = annot["query_set"]
            annot_uid = annot["annotation_uid"]
            annot_key = f"{clip_uid}_{annot_uid}_{query_set}"
            query_frame = annot["query_frame"]
            visual_crop = annot["visual_crop"]
            ...
            # TODO: just add my predict result to json
            
            output = inference_video(config, 
                                                     model, 
                                                     clip_path, 
                                                     query_frame, 
                                                     visual_crop,
...
# for debug
def get_query(config, clip_path, query_frame, visual_crop, device):
    ...
    # get query
    query = load_query(config, clip_reader, visual_crop, clip_path, debug=True)     # [c,h,w]
    return query
...
def inference_video(config, model, clip_path, query_frame, visual_crop, save_path, device):
    ...
    ret_bboxes, ret_scores, exist_probs = [], [], []
    exist_prob_flag = False
    for i in range(inference_time):
        ...
        if 'exist_prob' in preds.keys():
            exist_prob = preds['exist_prob'].reshape(-1).detach().cpu()
            exist_probs.append(exist_prob[:num_frames])
            exist_prob_flag = True
        # to watch video
        if config.debug: #and device == torch.device("cuda:0"):
            if exist_prob_flag:
                preds_top['exist_prob'] = exist_prob
    ...
    if exist_prob_flag:
        exist_probs = torch.cat(exist_probs, dim=0)
    return (ret_bboxes, ret_scores, exist_probs) if exist_prob_flag else (ret_bboxes, ret_scores)
...
def get_top_predictions(config, preds, num_frames, oshape):
    ...
    save_exist_prob = False
    if 'exist_prob' in preds.keys():
        pred_exist_prob = preds['exist_prob']
        save_exist_prob = True
