...
class Task:
    ...
    def run(self, config, device, model):
        ...
        for key, annot in zip(self.keys, self.annots):
            ...
            cropped_image = self.tfm(cropped_image)
            ...
            for peak in peaks:
                ...
                with torch.no_grad():
                    ...
                    cropped_image_features = model.encode_image(cropped_image.unsqueeze(0).to(device))
                    ...
                    # print()
                    peak_sim.append((peak, cosine_similarity(cropped_image_features.cpu().numpy(), image_features.cpu().numpy())[0][0]))
...
# start    
            frame_with_largest_similarity = max(peak_sim, key=lambda item: item[1])[0]
            # print(frame_with_largest_similarity)
            recent_peak = frame_with_largest_similarity
