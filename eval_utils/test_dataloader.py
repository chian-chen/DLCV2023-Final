...
def load_query(config, clip_reader, visual_crop, clip_path, debug=False):
    '''
    ...
    if debug:
        d_query = ImageDraw.Draw(query)
        d_query.rectangle([(bbox[0], bbox[1]), (bbox[2], bbox[3])], fill=None, outline="red")
        return query
