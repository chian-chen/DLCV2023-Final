...
def train_epoch(config, loader, model, optimizer, schedular, scaler, epoch, output_dir, device, rank=0, wandb_run=None, ddp=True):
    time_meters = exp_utils.AverageMeters()     # add and update loss by it
    loss_meters = exp_utils.AverageMeters()     # add and update loss by it
    
    train_utils.set_model_train(config, model, ddp)     # TODO: fix other parameters
    ...
    for batch_idx, sample in enumerate(loader):
        ...
        losses = loss_utils.get_losses_existence(config, preds, sample)
        total_loss = 0.0
        ...
        loss_val = 0
        loss_avg = 0
        # print(loss_meters.average_meters)
        for k, v in loss_meters.average_meters.items():
            loss_val = v.val
            loss_avg = v.avg
...
def validate(config, loader, model, epoch, output_dir, device, rank=0, wandb_run=None, ddp=True):
    model.eval()
    metrics = {}
    ...
    with torch.no_grad():
        for batch_idx, sample in enumerate(loader):
            ...
            if batch_idx % config.eval_vis_freq == 0: #batch_idx % config.eval_vis_freq == 0 and rank == 0:
                # vis_utils.vis_pred_clip(sample=sample,
                #                         pred=preds_top,
                #                         iter_num=batch_idx,
                #                         output_dir=output_dir,
    ...
    return torch.tensor(metrics['iou']).mean().item(), torch.tensor(metrics['prob_accuracy']).mean().item(), torch.tensor(metrics['prob_exist_accuracy']).mean().item()
...
def val_performance(config, preds, gts, prob_theta=0.5):
    ...
    exist_prob = preds["exist_prob"]# [b,t] after sigmoid
    ...
    exist_prob = rearrange(exist_prob, 'b t -> (b t)')
    ...
    # check my existence is better than prob_acdcuracy
    ...
    prob_exist_accuracy = ((torch.sigmoid(exist_prob) > prob_theta) == gt_prob.bool()).float().mean()
    prob_exist_accuracy_2 = ((torch.sigmoid(exist_prob) > 0.6) == gt_prob.bool()).float().mean()
    prob_exist_accuracy_3 = ((torch.sigmoid(exist_prob) > 0.7) == gt_prob.bool()).float().mean()
    prob_exist_accuracy_4 = ((torch.sigmoid(exist_prob) > 0.65) == gt_prob.bool()).float().mean()
    ...
    loss = {
        ...
        'prob_exist_accuracy': prob_exist_accuracy.item(),
        'prob_exist_accuracy_0.6': prob_exist_accuracy_2.item(),
        'prob_exist_accuracy_0.7': prob_exist_accuracy_3.item(),
        'prob_exist_accuracy_0.65': prob_exist_accuracy_4.item(),
    }

    # get top prediction
    pred_prob = rearrange(pred_prob, '(b t) -> b t', b=b, t=t)           # [b,t]
    pred_bbox = rearrange(pred_bbox, '(b t) c -> b t c', b=b, t=t)       # [b,t,4]
    exist_prob = rearrange(exist_prob, '(b t)-> b t', b=b, t=t)       # [b,t,4]
    pred_top ={
        'bbox': pred_bbox,
        'prob': pred_prob,
