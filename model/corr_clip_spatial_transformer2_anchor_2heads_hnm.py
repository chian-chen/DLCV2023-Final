...
from model.pred_head import ExistPredHead
...
class ClipMatcher(nn.Module):
    def __init__(self, config) -> None:
        ...
        self.num_transformer = config.model.num_transformer # default 3
        ...
        # TODO: if your vram is large enough, turn this to False
        self.out_of_memory = True
        
        self.input_from_spatial = True if config.model.input_from == "s" else False
        self.input_from_spatial_temporal = True if config.model.input_from == "st" else False
        self.side_len = 32 if self.input_from_spatial else 8
        exist_config = {}
        if self.input_from_spatial:
            exist_config = {
                "hidden_size": 512,
                "num_heads": 8,
                "num_layers": 2,
        ...
        self.exist_head_appear = self.input_from_spatial or self.input_from_spatial_temporal
        if self.exist_head_appear:
            self.exist_pred_head = ExistPredHead(input_size=256, output_size=1,side_len=self.side_len, config=exist_config, from_spatial=self.input_from_spatial, temporal=config.model.exist_head_temporal)
    ...
    def forward(self, clip, query, query_frame_bbox=None, training=False, fix_backbone=True):
        ...
        if fix_backbone:
            with torch.no_grad():
                ...
                # TODO: extract (b t c h w) to ((b t) c h w)
                if self.out_of_memory:
                    clip = rearrange(clip, 'b t c h w -> (b t) c h w')
                    clip_feat = None
                    bt = clip.shape[0]
                    for i in range(0,bt,b):
                        clip_slice = clip[i:min(i+b, bt),...]
                        clip_feat_slice = self.extract_feature(clip_slice)
                        if i == 0:
                            clip_feat = clip_feat_slice
                        else:
                            clip_feat = torch.cat([clip_feat, clip_feat_slice], dim=0)
                else:
                    clip = rearrange(clip, 'b t c h w -> (b t) c h w')
        ...
        if torch.is_tensor(query_frame_bbox) and self.config.train.use_query_roi:   # This wont be used. It is for COCOFormer
        ...
        spatial_clip_feat = rearrange(clip_feat, 'b (h w) c -> b c h w', h=h, w=w)                               # [b*t,c,h,w]

        # TODO: add existence head after spatial correspondence
        if self.input_from_spatial:
            exist_head = rearrange(spatial_clip_feat, '(b t) c h w -> b t h w c', t=t, b=b)
            exist_prob = self.exist_pred_head(exist_head)


        # down-size features and find spatial-temporal correspondence
        for head in self.down_heads:
            spatial_clip_feat = head(spatial_clip_feat)
            if list(spatial_clip_feat.shape[-2:]) == [self.resolution_transformer]*2: # if [h,w] == [8,8]
                _clip_feat = rearrange(spatial_clip_feat, '(b t) c h w -> b (t h w) c', b=b) + self.pe_3d
                mask = self.get_mask(_clip_feat, t)
                for layer in self.feat_corr_transformer:
                    _clip_feat = layer(_clip_feat, src_mask=mask)
                _clip_feat = rearrange(_clip_feat, 'b (t h w) c -> (b t) c h w', b=b, t=t, h=self.resolution_transformer, w=self.resolution_transformer)
                break
        
        # TODO: add existence head after spatial correspondence
        if self.input_from_spatial_temporal:
            exist_head = rearrange(_clip_feat, '(b t) c h w -> b t h w c', t=t, b=b)
            exist_prob = self.exist_pred_head(exist_head)

        # refine anchors
        anchors_xyhw = self.anchors_xyhw.to(_clip_feat.device)                   # [N,4]
        anchors_xyxy = self.anchors_xyxy.to(_clip_feat.device)                   # [N,4]
        anchors_xyhw = anchors_xyhw.reshape(1,1,-1,4)                           # [1,1,N,4]
        anchors_xyxy = anchors_xyxy.reshape(1,1,-1,4)                           # [1,1,N,4]
        
        # add residual feat from spatial
        clip_feat = spatial_clip_feat + _clip_feat
