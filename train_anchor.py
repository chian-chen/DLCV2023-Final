...
from func.train_existence import train_epoch, validate
...
def parse_args():
    ...
    parser.add_argument(
        "--ddp", default=False, action="store_true"
    )
    parser.add_argument(
        "--wandb", default=True, action="store_false"
    )
...
def main():
    ...
    if args.wandb:
        ...
        wandb_run = wandb.init(project=wandb_proj_name, group=wandb_name, entity='wmnlmec', name=config.wandb_name)#, name='smooth-puddle-94', resume=True)
        wandb.config.update({
    ...
    scaler = torch.cuda.amp.GradScaler()    # TODO: is this needed?
    ...
    if config.train.fix_pretrained:
        try:
            model = train_utils.load_pretrain(
                model,  pretrain_cpt_name=config.train.pretrain_cpt_name, strict=False, device=device,
            )
    ...
    if config.train.resume:
        try:
            model, optimizer, schedular, scaler, ep_resume, best_iou, best_prob = train_utils.resume_training(
                                                                                ...
                                                                                output_dir, strict=False,
                                                                                cpt_name=config.train.resume_cpt_name,
                                                                               )
    ...
    # train_sampler = torch.utils.data.distributed.DistributedSampler(train_data) # could disable it
    ...
    val_loader = torch.utils.data.DataLoader(val_data,      # why validation no sampler?
    ...
    for epoch in range(start_ep, end_ep):
        ...
        train_utils.save_checkpoint(
                {
                    'epoch': epoch + 1,
                    'state_dict': train_utils.store_trainable(model,config),
        ...
        if len(output) == 3:
            iou, prob, exist_prob = output
            if exist_prob > best_prob and exist_prob > prob:
                best_prob = prob
                # if local_rank == 0:
                train_utils.save_checkpoint(
                {
        ...
        logger.info('Rank {}, best iou: {} (current {}), best probability accuracy: {} (current {})'.format(0, best_iou, iou, best_prob, prob))
        torch.cuda.empty_cache()
