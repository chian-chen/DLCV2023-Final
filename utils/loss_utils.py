...
# modified, no file named model.corr_clip_anchor, and no parameter named default_aspect_ratios
...
def get_losses_existence(config, preds, gts):
    pred_exist = preds["exist_prob"]
    gt_exist = gts["clip_with_bbox"]
    pred_exist = torch.sigmoid(pred_exist)
    bce_loss = F.binary_cross_entropy_with_logits(pred_exist, gt_exist)
    loss = {
        "loss_exist_bce": bce_loss
    }
    return loss
