...
def vis_pred_scores(sample, pred, iter_num, output_dir, subfolder='train'):
    ...
    if 'exist_prob' in pred.keys():
        exist_prob = pred['exist_prob'].detach().cpu()            # [B,T]
    ...
    for i in range(B):
        ...
        if 'exist_prob' in pred.keys():
            cur_exist_prob = torch.sigmoid(exist_prob[i]).numpy()
            plt.plot(x, cur_exist_prob, marker=None, color='g', label='pred')
        if 'prob_refine' in pred.keys():
...
def vis_pred_clip_inference(clips, queries, pred, save_path, iter_num):
    ...
    #     print('exist_prob')
    #     T = prob.shape[0]
    #     exist_prob = torch.sigmoid(pred['exist_prob'])
    #     x = np.arange(T)
    #     plt.plot(x, prob,  marker=None, color='b', label='pred')
    #     plt.plot(x, exist_prob, marker=None, color='g', label='pred')
    #     plt.savefig(save_path+'_pred_curve.jpg')
